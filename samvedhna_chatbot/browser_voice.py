            ...
            (function() {
                ...
                function checkBrowserCompatibility() {
                    try {
                        ...
                        if (!('speechSynthesis' in window)) {
                            updateStatus('Speech synthesis not supported in this browser. Please use Chrome.', true);
                            return false;
                        }

                        // Check for speech recognition
                        if (!('webkitSpeechRecognition' in window)) {
                            updateStatus('Speech recognition not supported in this browser. Please use Chrome.', true);
                            return false;
                        }
                ...
                async function requestMicrophonePermission() {
                    try {
                        ...
                        if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
                            updateStatus('Please allow microphone access in your browser settings to use voice input', true);
                        } else if (error.name === 'NotFoundError') {
                            updateStatus('No microphone found. Please connect a microphone and try again', true);
                        } else if (error.name === 'NotReadableError') {
                            updateStatus('Microphone is in use by another application', true);
                        } else {
                            ...
                            return new Promise((resolve) => {
                                ...
                                const checkVoices = () => {
                                    ...
                                    if (voices.length > 0) {
                                        ...
                                        updateStatus('No voices available. Please check your browser settings.', true);
                                        resolve(false);
                                    }
                                };
                ...
                function initSpeechRecognition() {
                    try {
                        ...
                        if (!SpeechRecognition) {
                            updateStatus('Speech recognition not supported. Please use Chrome.', true);
                            return false;
                        }
                ...
                window.addEventListener('message', function(event) {
                    try {
                        ...
                        // Verify message origin and structure
                        if (!data || !data.type) {
                            return;
                        }
